...
<style>
*{box-sizing:border-box}body{margin:0;background:#0c1020;color:#edf1ff;font:15px ui-sans-serif,system-ui,sans-serif}main{max-width:1120px;margin:auto;padding:36px 22px}h1{margin:0;font-size:28px}p{color:#aab4d4}.grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:14px;margin:24px 0}.card{background:#151b31;border:1px solid #293354;border-radius:14px;padding:18px}.label{color:#9eabd0;font-size:13px}.info{display:inline-grid;place-items:center;width:16px;height:16px;border:1px solid #7180ad;border-radius:50%;font-size:11px;cursor:help}.value{font-size:27px;font-weight:700;margin-top:7px}.panel{background:#151b31;border:1px solid #293354;border-radius:14px;padding:20px;margin:14px 0}h2{font-size:16px;margin:0 0 6px}.legend{font-size:12px;color:#aab4d4;margin-bottom:10px}.swatch{display:inline-block;width:10px;height:10px;border-radius:50%;margin-right:5px}.tokens{background:#70e3c2}.bars{display:grid;gap:11px}.bar-row{display:grid;grid-template-columns:150px 1fr 52px;gap:10px;align-items:center}.track{height:10px;background:#242d4b;border-radius:99px;overflow:hidden}.fill{height:100%;border-radius:99px;background:linear-gradient(90deg,#70e3c2,#728cff)}.chart{height:180px;width:100%}.note{font-size:13px;color:#aab4d4}svg{overflow:visible}.axis{stroke:#354163}.line{fill:none;stroke:#70e3c2;stroke-width:3}.dot{fill:#70e3c2}@media(max-width:550px){.bar-row{grid-template-columns:105px 1fr 42px}}
</style></head><body><main><h1>Codex Smart Router</h1><p id="subtitle">Loading private local audit data…</p><section id="cards" class="grid"></section><section class="panel"><h2>Turns and tokens over time</h2><svg class="chart" id="trend" viewBox="0 0 900 180" preserveAspectRatio="none"></svg></section><section class="panel"><h2>Model distribution</h2><div id="models" class="bars"></div></section><section class="panel"><h2>Routing distribution</h2><div id="tiers" class="bars"></div></section><p class="note" id="note"></p></main><script>
...
function trend(hourly){const entries=Object.entries(hourly||{}).filter(([k])=>k!=='unknown').sort(([a],[b])=>a.localeCompare(b)).slice(-48);const svg=document.querySelector('#trend');if(!entries.length){svg.innerHTML='';return}const w=900,h=150,max=Math.max(...entries.map(([,v])=>v.totalTokens),1),points=entries.map(([,v],i)=>[(entries.length===1?w/2:i*w/(entries.length-1)),h-v.totalTokens/max*(h-12)+6]);svg.innerHTML='<path class="axis" d="M0 '+h+'H'+w+'"/><polyline class="line" points="'+points.map(p=>p.join(',')).join(' ')+'"/>'+points.map(([x,y],i)=>'<circle class="dot" cx="'+x+'" cy="'+y+'" r="4"><title>'+new Date(entries[i][0]).toLocaleString()+': '+number(entries[i][1].totalTokens)+' tokens, '+entries[i][1].turns+' turns</title></circle>').join('')}
async function load(){const d=await fetch('/api/dashboard').then(r=>r.json()),e=d.estimate,c=[['Turns','Completed usage records',number(d.usage.turns)],['Tokens processed','Input + output + reasoning tokens',number(d.usage.totalTokens)+' tokens'],['Routing coverage','Routes handled by a selected tier',d.convenience.routingCoveragePercent.toFixed(1)+'%'],['Classifier overhead','Average classifier latency per turn',Math.round(d.convenience.classifierOverheadMsPerTurn)+' ms/turn'],['Estimated savings vs '+e.baselineModel,'Estimated API cost difference',money(e.estimatedSavings)],['Routed cost','Estimated total API cost',money(e.routedCost)]];document.querySelector('#cards').innerHTML=c.map(([l,i,v])=>'<article class="card" title="'+i+'"><div class="label">'+l+' <span class="info" aria-label="'+i+'">i</span></div><div class="value">'+v+'</div></article>').join('');document.querySelector('#subtitle').textContent='Last refresh: '+new Date().toLocaleTimeString()+' · tokens are raw token units · chart uses hourly buckets for the last 48 hours';document.querySelector('#note').textContent=e.note;bars('#models',d.models);bars('#tiers',d.tiers);trend(d.hourly)}load();setInterval(load,15000);
</script><script>const chartUnits=()=>{const svg=document.querySelector('#trend');if(!svg||svg.querySelector('.unit-label'))return;svg.insertAdjacentHTML('afterbegin','<text class="unit-label" fill="#aab4d4" font-size="12" x="8" y="16">tokens / hour</text><text class="unit-label" fill="#aab4d4" font-size="12" x="850" y="174">time →</text>')};setInterval(chartUnits,500);</script></body></html>`;
